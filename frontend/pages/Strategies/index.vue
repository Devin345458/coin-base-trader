...
export default {
  ...
  data () {
    return {
      headers: [
        ...
        { text: 'Trailing Buy Price', value: 'trailingBuyPrice' },
        { text: 'Trailing Sell Price', value: 'trailingSellPrice' },
  ...
  async mounted () {
    this.loading = true
    const { data: { strategies, message, problems }, status } = await this.$axios.get('/v1/strategy', {}).catch(e => e)
    this.loading = false
  ...
  methods: {
    ...
    async deleteStrat (id) {
      if (await this.$confirm('Do you really want to delete this strategy?', { title: 'Danger Zone', icon: 'mdi-alert-circle-outline', color: 'error' })) {
        ...
        const { data: { message, problems }, status } = await this.$axios.delete('/v1/strategy/delete', {
          params: {
            id
          }
        }).catch(e => e)
