/**
 * @name global.sails.helpers.sendTemplateEmail
 * @type {{exits: {success: {outputType: {loggedInsteadOfSending: string}, outputFriendlyName: string, outputDescription: string}}, inputs: {template: {description: string, extendedDescription: string, type: string, required: boolean, example: string}, layout: {custom: (function(*=)), description: string, defaultsTo: string}, ensureAck: {description: string, extendedDescription: string, type: string, defaultsTo: boolean}, bcc: {description: string, example: [string]}, attachments: {description: string, whereToGet: {description: string}, defaultsTo: [], example: [{name: string, contentBytes: string, type: string}]}, toName: {description: string, example: string}, subject: {description: string, defaultsTo: string, example: string}, templateData: {description: string, extendedDescription: string, type: {}, defaultsTo: {}}, fromName: {description: string, example: string}, from: {isEmail: boolean, description: string, example: string}, to: {isEmail: boolean, description: string, extendedDescription: string, required: boolean, example: string}}, fn: (function({template?: *, templateData?: *, to?: *, toName?: *, subject: *, from?: *, fromName?: *, layout?: *, ensureAck?: *, bcc?: *, attachments: *}): {loggedInsteadOfSending: boolean}), description: string, extendedDescription: string, friendlyName: string}}
 */
module.exports = {
  ...
  async fn ({ template, templateData, to, toName, subject, from, fromName, layout, ensureAck, bcc, attachments }) {
    ...
    if (dontActuallySend) {
      ...
      if (!sails.config.mail.apiKey) {
        throw new Error(
          'Cannot deliver email to "' + to + '" because:\n' +
          (() => {
            const problems = []
            if (!sails.config.mail.apiKey) {
              problems.push(' • Mailgun secret is missing from this app\'s configuration (`sails.config.mail.apiKey`)')
            }
      ...
      const messageData = {
        ...
        from: from || sails.config.mail.from,
        fromName,
        secret: sails.config.mail.apiKey,
        domain: sails.config.mail.domain,
        attachments
      }

      const deferred = sails.helpers.mailgun.sendHtmlEmail.with(messageData)
      if (ensureAck) {
        await deferred
      } else {
